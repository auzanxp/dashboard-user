...
export default function Button({ children, color, ...props }) {
    ...
    if (!color) clasName.push(' bg-indigo-400 hover:bg-indigo-500');

    if (color === 'yellow') clasName.push(' bg-amber-400 hover:bg-amber-500');
    ...
    if (props.type === 'link') {
        return (
            <Link to={props.href} className={`${clasName.join('')} w-fit gap-x-2 [&>svg]:w-5 [&>svg]:h-5`}>
                {children}
            </Link>
        );
    } else {
